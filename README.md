...
- Long issue summaries and statuses stay compact and can optionally scroll to reveal their complete text in inline issues, `jira-issue` blocks, and list search results.
...
- Overflow animation is disabled by default. It can be enabled from `Settings > Jira Issue > Rendering`, only runs when text actually overflows, and respects the operating system's reduced-motion preference.
